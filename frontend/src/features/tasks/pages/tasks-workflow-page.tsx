...
export function TasksWorkflowPage() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <Card className="border-border/70"><CardContent className="flex flex-col gap-3 pt-5 lg:flex-row"><div className="relative flex-1"><Search className="absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="جست‌وجوی تسک..." className="ps-9" /></div><Select value={projectId} onChange={(event) => setProjectId(event.target.value)}><option value="">همهٔ پروژه‌ها</option>{projects?.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</Select><Select value={value} onChange={(event) => setValue(event.target.value)}><option value="">همهٔ ارزش‌ها</option><option value="high">{VALUE_LABEL.high}</option><option value="medium">{VALUE_LABEL.medium}</option><option value="low">{VALUE_LABEL.low}</option></Select></CardContent></Card>
