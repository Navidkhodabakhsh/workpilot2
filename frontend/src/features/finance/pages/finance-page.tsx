...
function DocumentDialog() {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button><Plus className="size-4" />ثبت سند</Button></DialogTrigger><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>سند درآمد یا هزینه</DialogTitle><DialogDescription>هر رکورد مانند یک سند مالی با گروه، تاریخ، مبلغ و طرف حساب ثبت می‌شود.</DialogDescription></DialogHeader><form className="grid grid-cols-1 gap-4 sm:grid-cols-2" onSubmit={form.handleSubmit((values) => mutation.mutate(values))}><div className="flex flex-col gap-2"><Label htmlFor="new-finance-entry-type" required>نوع سند</Label><Select id="new-finance-entry-type" {...form.register("entry_type")}><option value="income">درآمد</option><option value="expense">هزینه</option></Select></div><div className="flex flex-col gap-2"><Label htmlFor="new-finance-entry-category" required>گروه</Label><Select id="new-finance-entry-category" {...form.register("category_id")}><option value="">انتخاب گروه</option>{categories?.filter((item) => item.entry_type === entryType).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select>{form.formState.errors.category_id && <p className="text-xs text-danger">{form.formState.errors.category_id.message}</p>}</div><div className="flex flex-col gap-2 sm:col-span-2"><Label htmlFor="new-finance-entry-title" required>عنوان سند</Label><Input id="new-finance-entry-title" {...form.register("title")} />{form.formState.errors.title && <p className="text-xs text-danger">{form.formState.errors.title.message}</p>}</div><div className="flex flex-col gap-2"><Label htmlFor="new-finance-entry-amount" required>مبلغ (تومان)</Label><Controller control={form.control} name="amount" render={({ field }) => <CurrencyInput id="new-finance-entry-amount" value={field.value} onChange={field.onChange} />} />{form.formState.errors.amount && <p className="text-xs text-danger">{form.formState.errors.amount.message}</p>}</div><div className="flex flex-col gap-2"><Label htmlFor="new-finance-entry-date" required>تاریخ سند</Label><Controller control={form.control} name="document_date" render={({ field }) => <JalaliDateInput id="new-finance-entry-date" value={field.value} onChange={field.onChange} />} /></div><div className="flex flex-col gap-2"><Label htmlFor="new-finance-entry-project">پروژه</Label><Select id="new-finance-entry-project" {...form.register("project_id")}><option value="">بدون پروژه</option>{projects?.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</Select></div><div className="flex flex-col gap-2"><Label htmlFor="new-finance-entry-counterparty">طرف حساب</Label><Input id="new-finance-entry-counterparty" {...form.register("counterparty")} /></div><div className="flex flex-col gap-2"><Label htmlFor="new-finance-entry-document-number">شماره سند</Label><Input id="new-finance-entry-document-number" {...form.register("document_number")} /></div><div className="flex flex-col gap-2 sm:col-span-2"><Label htmlFor="new-finance-entry-description">شرح</Label><Textarea id="new-finance-entry-description" {...form.register("description")} /></div>{mutation.isError && <p className="text-sm text-danger sm:col-span-2">ثبت سند انجام نشد.</p>}<Button type="submit" className="sm:col-span-2" disabled={mutation.isPending}>{mutation.isPending ? "در حال ثبت..." : "ثبت سند مالی"}</Button></form></DialogContent></Dialog>
}
...
function CategoryDialog() {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline"><Tags className="size-4" />گروه جدید</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>ساخت گروه مالی</DialogTitle><DialogDescription>گروه‌های دلخواه برای دسته‌بندی اسناد بسازید.</DialogDescription></DialogHeader><div className="flex flex-col gap-4"><div className="flex flex-col gap-2"><Label htmlFor="new-finance-category-type" required>نوع</Label><Select id="new-finance-category-type" value={type} onChange={(event) => setType(event.target.value as FinanceEntryType)}><option value="expense">هزینه</option><option value="income">درآمد</option></Select></div><div className="flex flex-col gap-2"><Label htmlFor="new-finance-category-name" required>نام گروه</Label><Input id="new-finance-category-name" value={name} onChange={(event) => setName(event.target.value)} /></div><div className="flex flex-col gap-2"><Label htmlFor="new-finance-category-color" required>رنگ گروه</Label><Input id="new-finance-category-color" type="color" value={color} onChange={(event) => setColor(event.target.value)} className="h-11" /></div><Button disabled={name.trim().length < 2 || mutation.isPending} onClick={() => mutation.mutate()}>ذخیره گروه</Button></div></DialogContent></Dialog>
}
