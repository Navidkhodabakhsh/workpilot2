...
**۱) ثبت‌نام (اولین بار — ساخت سازمان تازه + اولین دپارتمان آن + اولین کاربر به‌عنوان org_admin):**

> از این نسخه به بعد، `department_name` هم در بدنهٔ ثبت‌نام **اجباری** است — هر سازمان تازه باید همان لحظهٔ ساخت، حداقل یک دپارتمان داشته باشد (بخش «توضیح تمام جداول»، جدول `departments` را ببینید). سرویس `signup` (`backend/app/services/auth.py`) هم‌زمان با ساخت `Organization`، این اولین `Department` را هم می‌سازد و کاربر تازه (org_admin) را عضو همان دپارتمان می‌کند.
